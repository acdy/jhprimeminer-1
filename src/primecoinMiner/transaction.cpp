...
void bitclient_addVarIntFromStream(stream_t* msgStream, uint64_t varInt)
{
	if( varInt <= 0xFC )
	{
		stream_writeU8(msgStream, (uint8_t)varInt);
		return;
	}
	else if( varInt <= 0xFFFF )
	{
		stream_writeU8(msgStream, 0xFD);
		stream_writeU16(msgStream, (uint16_t)varInt);
		return;
	}
	else if( varInt <= 0xFFFFFFFF )
	{
		stream_writeU8(msgStream, 0xFE);
		stream_writeU32(msgStream, (uint32_t)varInt);
		return;
	}
	else
	{
...
{
	for(uint32_t i=0; i<tx->tx_in->objectCount; i++)
		free(tx->tx_in->objects[i]);
	for(uint32_t i=0; i<tx->tx_out->objectCount; i++)
...
bitclientVIn_t* bitclient_addTransactionInput(bitclientTransaction_t* tx)
{
	...
	vin->scriptSignatureData = (uint8_t*)(vin+1);
...
bitclientVOut_t* bitclient_addTransactionOutput(bitclientTransaction_t* tx)
{
	...
	vout->scriptPkData = (uint8_t*)(vout+1);
	simpleList_add(tx->tx_out,vout);
	return vout;
}

void bitclient_addScriptOpcode(bitclientVIn_t* vin, uint8_t opcode)
{
	vin->scriptSignatureData[vin->scriptSignatureLength] = opcode;
	vin->scriptSignatureLength++;
}

void bitclient_addScriptU32(bitclientVIn_t* vin, uint32_t v, bool hasLengthPrefix)
{
	if( v <= 0x7F )
	{
		if( hasLengthPrefix )
		...
		*(uint8_t*)&vin->scriptSignatureData[vin->scriptSignatureLength] = v;
	...
	else if( v <= 0x7FFF )
	{
		...
		*(uint8_t*)&vin->scriptSignatureData[vin->scriptSignatureLength+0] = (v>>0)&0xFF;
		*(uint8_t*)&vin->scriptSignatureData[vin->scriptSignatureLength+1] = (v>>8)&0xFF;
	...
	else if( v <= 0x7FFFFF )
	{
		...
		*(uint8_t*)&vin->scriptSignatureData[vin->scriptSignatureLength+0] = (v>>0)&0xFF;
		*(uint8_t*)&vin->scriptSignatureData[vin->scriptSignatureLength+1] = (v>>8)&0xFF;
		*(uint8_t*)&vin->scriptSignatureData[vin->scriptSignatureLength+2] = (v>>16)&0xFF;
	...
	else if( v <= 0x7FFFFFFF )
	{
		...
		*(uint8_t*)&vin->scriptSignatureData[vin->scriptSignatureLength+1] = (v>>8)&0xFF;
		*(uint8_t*)&vin->scriptSignatureData[vin->scriptSignatureLength+2] = (v>>16)&0xFF;
		*(uint8_t*)&vin->scriptSignatureData[vin->scriptSignatureLength+3] = (v>>24)&0xFF;
	...
	else
	{
		...
		*(uint8_t*)&vin->scriptSignatureData[vin->scriptSignatureLength+2] = (v>>16)&0xFF;
		*(uint8_t*)&vin->scriptSignatureData[vin->scriptSignatureLength+3] = (v>>24)&0xFF;
		*(uint8_t*)&vin->scriptSignatureData[vin->scriptSignatureLength+4] = 0;
		vin->scriptSignatureLength += 5;
	}

}

void bitclient_addScriptData(bitclientVIn_t* vin, uint8_t* data, uint32_t length, bool hasLengthPrefix)
{
...
void bitclient_addScriptOpcode(bitclientVOut_t* vout, uint8_t opcode)
{
	vout->scriptPkData[vout->scriptPkLength] = opcode;
	vout->scriptPkLength++;
}

void bitclient_addScriptData(bitclientVOut_t* vout, uint8_t* data, uint32_t length)
{
	if( length <= 100 )
...
bitclientTransaction_t* bitclient_createCoinbaseTransactionFromSeed(uint32_t seed, uint32_t seed_userId, uint32_t blockHeight, uint8_t* pubKeyRipeHash, uint64_t inputValue)
{
	...
	bitclient_addScriptData(vInput, (uint8_t*)&seed, 4, true);
	uint32_t seed2 = seed_userId ^ 0xf83220df + seed_userId * 29481 + seed_userId * 3;
	bitclient_addScriptData(vInput, (uint8_t*)&seed2, 4, true);
	bitclient_addScriptData(vInput, (uint8_t*)"jhPrimeminer", 12, true);
	bitclient_addScriptData(vInput, (uint8_t*)"/P2SH/", 6, false); // coinbase flags...
	// create output
	...
	bitclient_addScriptData(vOutput, (uint8_t*)pubKeyRipeHash, 20);
...
void bitclient_generateTxHash(bitclientTransaction_t* tx, uint8_t* txHash)
{
	...
	int32_t transactionDataLength = 0;
	uint8_t* transactionData = (uint8_t*)streamEx_map(streamTXData, &transactionDataLength);
...
void bitclient_calculateMerkleRoot(uint8_t* txHashes, uint32_t numberOfTxHashes, uint8_t merkleRoot[32])
{
	...
	else
	{
		...
		uint8_t hashData[256*32]; // space for 256 hashes
		uint32_t hashCount = 0; // number of hashes written to hashData
		uint32_t hashReadIndex = 0; // index of currently processed hash
		uint32_t layerSize[10] = {0}; // up to 16 layers (which means 2^10 hashes are possible)
		layerSize[0] = numberOfTxHashes;
		for(uint32_t i=0; i<numberOfTxHashes; i++)
		{
		...
		for(uint32_t f=0; f<10; f++)
		{
			...
			for(uint32_t i=0; i<layerSize[f]; i += 2)
			{
				uint8_t hashOut[32];
				sha256_context sha256_ctx;
				sha256_starts(&sha256_ctx);
