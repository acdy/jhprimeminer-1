...
stream_t *stream_create(streamSettings_t *settings, void *object)
{
	...
	if( stream->settings->initStream != NULL )
...
void stream_writeBits(stream_t* stream, uint8_t* bitData, uint32_t bitCount)
{
	...
	{
		uint32_t srcByteIndex = i/8;
		uint32_t srcBitIndex = i&7;
		uint8_t bitValue = (bitData[srcByteIndex]>>srcBitIndex)&1;
		// append bit
		uint32_t dstByteIndex = stream->bitIndex/8;
		uint32_t dstBitIndex = stream->bitIndex&7;
		stream->bitIndex++;
		if( bitValue )
...
void stream_readBits(stream_t* stream, uint8_t* bitData, uint32_t bitCount)
{
	...
	while( i < bitCount )
	{
		...
		{
			// get source bit
			uint32_t srcByteIndex = stream->bitReadIndex/8;
			uint32_t srcBitIndex = stream->bitReadIndex&7;
			stream->bitReadIndex++;
			stream->bitReadBufferState--;
			uint8_t bitValue = (stream->bitReadBuffer[srcByteIndex]>>srcBitIndex)&1;
			// overwrite dst bit
			uint32_t dstByteIndex = i/8;
			uint32_t dstBitIndex = i&7;
			stream->bitIndex++;
			if( bitValue )
...
uint32_t stream_copy(stream_t* dest, stream_t* source, uint32_t length)
{
	...
	// alloc copy buffer
	uint8_t* copyBuffer = (uint8_t*)malloc(copySize);
	// start copy loop
	uint32_t copyAmount = 0;
	while( length > 0 )
	{
		uint32_t stepCopy = std::min(length, copySize);
		uint32_t bytesRead = stream_readData(source, copyBuffer, stepCopy);
		uint32_t bytesWritten = stream_writeData(dest, copyBuffer, stepCopy);
		uint32_t tBytes = std::min(bytesRead,bytesWritten);
		if( tBytes == 0 )
...
//
//uint32_t __fastcall streamEx_memoryRange_readData(void *object, void *buffer, uint32_t len)
//{
//	return 0;
//}
//
//uint32_t __fastcall streamEx_memoryRange_writeData(void *object, void *buffer, uint32_t len)
//{
//	return 0;
//}
//
//uint32_t __fastcall streamEx_memoryRange_getSize(void *object)
//{
//	return 0;
//}
//
//void __fastcall streamEx_memoryRange_setSize(void *object, uint32_t size)
//{
//
//}
//
//uint32_t __fastcall streamEx_memoryRange_getSeek(void *object)
//{
//	return 0;
//}
//
//void __fastcall streamEx_memoryRange_setSeek(void *object, int32_t seek, bool relative)
//{
//
//}
//
...
uint32_t __fastcall streamEx_dynamicMemoryRange_readData(void *object, void *buffer, uint32_t len)
{
	streamEx_dynamicMemoryRange_t* memoryRangeObj = (streamEx_dynamicMemoryRange_t*)object;
	uint32_t bytesToRead = std::min(len, memoryRangeObj->bufferSize - memoryRangeObj->bufferPosition);
	memcpy(buffer, memoryRangeObj->buffer + memoryRangeObj->bufferPosition, bytesToRead);
	memoryRangeObj->bufferPosition += bytesToRead;
	return bytesToRead;
}

uint32_t __fastcall streamEx_dynamicMemoryRange_writeData(void *object, void *buffer, uint32_t len)
{
	streamEx_dynamicMemoryRange_t* memoryRangeObj = (streamEx_dynamicMemoryRange_t*)object;
	...
	uint32_t overwriteSize = memoryRangeObj->bufferSize - memoryRangeObj->bufferPosition; // amount of bytes that can be written without exceeding the buffer size
	if( overwriteSize )
	{
	...
	uint32_t bufferBytesLeft = memoryRangeObj->bufferLimit - memoryRangeObj->bufferPosition;
	// need to enlarge buffer?
	if( bufferBytesLeft < nLen )
	{
		uint32_t enlargeSize = 0;
		uint32_t minimalEnlargeSize = nLen - bufferBytesLeft;
		// calculate ideal enlargement size (important)
		uint32_t idealEnlargeSize = memoryRangeObj->bufferLimit/4; // 25% of current buffer size
		...
		uint32_t maxEnlargeSize = memoryRangeObj->sizeLimit - memoryRangeObj->bufferLimit;
		enlargeSize = std::min(maxEnlargeSize, enlargeSize);
		if( enlargeSize )
		{
			// enlarge
			uint32_t newLimit = memoryRangeObj->bufferLimit + enlargeSize;
			uint8_t* newBuffer = (uint8_t*)malloc(newLimit);
...
uint32_t __fastcall streamEx_dynamicMemoryRange_getSize(void *object)
{
	streamEx_dynamicMemoryRange_t* memoryRangeObj = (streamEx_dynamicMemoryRange_t*)object;
	return memoryRangeObj->bufferSize;
}

void __fastcall streamEx_dynamicMemoryRange_setSize(void *object, uint32_t size)
{
	streamEx_dynamicMemoryRange_t* memoryRangeObj = (streamEx_dynamicMemoryRange_t*)object;
	if( memoryRangeObj->disallowWrite )
		return;
...
uint32_t __fastcall streamEx_dynamicMemoryRange_getSeek(void *object)
{
	streamEx_dynamicMemoryRange_t* memoryRangeObj = (streamEx_dynamicMemoryRange_t*)object;
	return memoryRangeObj->bufferPosition;
}

void __fastcall streamEx_dynamicMemoryRange_setSeek(void *object, int32_t seek, bool relative)
{
	streamEx_dynamicMemoryRange_t* memoryRangeObj = (streamEx_dynamicMemoryRange_t*)object;
...
stream_t* streamEx_fromMemoryRange(void *mem, uint32_t memoryLimit)
{
	...
	memoryRangeObj->buffer = (uint8_t*)mem;
...
stream_t* streamEx_fromDynamicMemoryRange(uint32_t memoryLimit)
{
	...
	memoryRangeObj->buffer = (uint8_t*)malloc(memoryRangeObj->bufferLimit);
	memoryRangeObj->bufferPosition = 0;
...
uint32_t __fastcall streamEx_substream_readData(void *object, void *buffer, uint32_t len)
{
	streamEx_substream_t* substream = (streamEx_substream_t*)object;
	uint32_t readLimit = substream->size - substream->currentOffset;
	...
	uint32_t realRead = stream_readData(substream->stream, buffer, len);
	substream->currentOffset += realRead;
	return realRead;
}

uint32_t __fastcall streamEx_substream_writeData(void *object, void *buffer, uint32_t len)
{	
#ifdef _WIN32
		__debugbreak();
#else
...
uint32_t __fastcall streamEx_substream_getSize(void *object)
{
	streamEx_substream_t* substream = (streamEx_substream_t*)object;
	return substream->size;
}

void __fastcall streamEx_substream_setSize(void *object, uint32_t size)
{
#ifdef _WIN32
		__debugbreak();
#else
...
uint32_t __fastcall streamEx_substream_getSeek(void *object)
{
	streamEx_substream_t* substream = (streamEx_substream_t*)object;
	return substream->currentOffset;
}

void __fastcall streamEx_substream_setSeek(void *object, int32_t seek, bool relative)
{
	streamEx_substream_t* substream = (streamEx_substream_t*)object;
...
void* streamEx_map(stream_t* stream, int32_t* size)
{
	...
	int32_t rSize = stream_getSize(stream);
	*size = rSize;
	if( rSize == 0 )
...
{
	for(uint32_t i=0; i<strSize-1; i++)
	{
